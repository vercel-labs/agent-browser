...
Use your real Chromium-based browser profile and active tabs through the Playwright MCP Bridge extension.

**Install extension:** <https://chromewebstore.google.com/detail/playwright-mcp-bridge/mmlmfjhmonkocbjadbfplnigmagldckm>

```bash
# Basic usage
agent-browser -p bridge snapshot -i
agent-browser -p bridge click @e2
agent-browser -p bridge screenshot --annotate
```

Or with environment variables:

```bash
export AGENT_BROWSER_PROVIDER=bridge
...
| ----------------------------------- | -------------------------------------------------------- | ---------------------------------- |
| `AGENT_BROWSER_BRIDGE_PORT`         | Local bridge relay port                                  | `9223`                             |
...
```bash
# WSL users: force opening connect URL via Windows host browser
...
- `AGENT_BROWSER_EXECUTABLE_PATH` still has highest opener priority when set.
