...
// ---------------------------------------------------------------------------
// File chooser upload without selector (no browser — expects error)
// ---------------------------------------------------------------------------

#[tokio::test]
async fn test_upload_without_selector_no_chooser_returns_error() {
    let mut state = DaemonState::new();
    let cmd = json!({ "id": "fc-1", "action": "upload", "files": ["./test.txt"] });
    let result = execute_command(&cmd, &mut state).await;
    assert_eq!(result["success"], false);
    let error = result["error"].as_str().unwrap_or("");
    // Browser auto-launches, then fails because no file chooser is open
    assert!(
        error.contains("No file chooser dialog is open"),
        "Expected 'No file chooser dialog is open', got: {}",
        error
    );
}

#[tokio::test]
async fn test_pending_file_chooser_preserved_on_selector_upload() {
    let mut state = DaemonState::new();
    state.pending_file_chooser = Some(42);
    let cmd = json!({
        "id": "fc-4",
        "action": "upload",
        "selector": "#input",
    ...
    assert_eq!(
        ...
        Some(42),
        "pending_file_chooser should be preserved when selector is used"
    );
}
