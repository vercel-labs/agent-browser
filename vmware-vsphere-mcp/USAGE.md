...
- Node.js 18+ (for supergateway stdio proxy)
...
> Claude Desktop 只支援 stdio 模式的 MCP server。`supergateway` 作為 stdio proxy，
> 將 HTTP endpoint 橋接成 Claude Desktop 可接受的格式。`npx -y` 會自動下載，不需另行安裝。

> 若設定檔已有其他 MCP server，只需把 `"vsphere": { ... }` 加入 `mcpServers` 物件即可。

若 MCP server 部署在其他主機（非 localhost），將 URL 改為對應 IP/port：

```json
"args": ["-y", "supergateway", "--streamableHttp", "http://192.168.1.100:8000/mcp"]
```
...
3. 寫入 Claude Desktop 設定檔（使用 supergateway 格式）
...
**Claude Desktop 啟動時出現 "Required" 錯誤（command 欄位缺失）**
- 舊版設定使用 `"type": "streamable-http"` 語法，Claude Desktop 不支援此格式
- 請改用 `supergateway` 格式（見上方步驟 4）
...
- 同步更新 Claude Desktop 設定中 supergateway 的 URL
