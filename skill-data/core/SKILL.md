...
Use `read [url]` when you need to consume documentation or other text pages rather than interact with a rendered UI. Omit the URL to read the rendered DOM of the active tab in the current browser session, including browser auth state and client-side updates. Explicit URL reads run HTTP directly before daemon/browser setup, even in a batch, and do not use browser cookies or alter a shared browser. Mixed batches apply launch options such as `--state` once before the first browser-dependent row, not again between rows. Only a policy-required `read` → `confirm` flow needs a browserless daemon to hold the pending command; it never reconfigures an existing daemon. Protocol support is checked explicitly rather than inferred from version labels. Only explicit-URL HTTP confirmation prompts carry `data.capabilities.readRequiresConfirmation: true`; `confirm`/`deny` require the exact ID and cannot consume a newer action through a stale ID. Bare DOM read prompts do not carry that marker. Reads use the current CLI/config rules and standard HTTP proxy environment variables. They send `Accept: text/markdown`, try the same URL with `.md` appended when the first response is not markdown, walk ancestor paths toward `/` to find the nearest `llms.txt` for a matching docs link, print markdown/plain text when available, and fall back to readable text extracted from HTML without launching Chrome. Add `--filter <text>` to narrow a page to matching heading sections, `--outline` for compact headings on one page, `--llms index` for a compact nearest-ancestor `llms.txt` link list, and `--llms full` only when you explicitly need `llms-full.txt`. With `--llms` or `--require-md`, omitting the URL uses the active tab URL because those modes depend on HTTP resources. With `--llms` or `--outline`, `--filter <text>` narrows links, sections, or headings. Add `--require-md` when you specifically want to verify markdown negotiation, `--raw` when you need the response body unchanged, and `--json` when you need metadata such as `source` and `contentType`. Global safeguards such as `--allowed-domains`, `--content-boundaries`, and `--max-output` also apply to read fetches and output.
...
`session info` is a read-only preflight: daemon state is separate from browser liveness, PID, effective profile, tabs, and ownership. Unknown values stay `null`, including bare/empty Chrome profile overrides. See [session management](references/session-management.md) for field meanings.
...
```bash
agent-browser open https://example.com
agent-browser record start demo.webm          # 30 output fps by default; .webm or .mp4
agent-browser snapshot -i
agent-browser click @e3
agent-browser record stop
```

`record start` attaches to the active tab as-is (no new context, no navigation unless you pass a URL). To record in a separate tab, run `tab new <url>` first. Recording needs `ffmpeg` on PATH (`brew install ffmpeg` / `apt install ffmpeg`); `agent-browser doctor` checks for it. `--fps` controls output playback (1 to 60), not capture rate. Chrome supplies frames on repaint and the recorder holds images through gaps; output FPS cannot establish smoothness. `record stop --json` separates capture timestamps, wall duration and frame-event rate from encoded playback. A successful start does not prove a frame has arrived yet. After a stop timeout, inspect `session info --json` → `data.runtime.recording.last` and match the `recordingId` from start; failed encodes retain failed receipts too.
