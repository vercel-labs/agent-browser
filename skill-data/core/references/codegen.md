...
A `select` records the option the browser settled on, not the text the command was given, because a label can match after whitespace normalization and no replay tool normalizes. A `check` or `uncheck` command that finds the control already in the requested state changes nothing. Codegen records an assertion for that state instead of a click, because a replayed click would clear the control. A scroll is recorded as the position the page reached, so repeated scrolls replay to the same place.
