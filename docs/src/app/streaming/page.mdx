...
Frames are delivered latest-first: when a client falls behind (slow network, slow render loop), stale frames are dropped and the client always receives the newest available frame instead of draining a backlog. Status, console, and tab messages flow through a separate ordered channel and are not subject to frame dropping.
...
`maxFps` caps how many frames per second the server sends to this client (1 to 120). Set `0` to remove the cap (the default). Each client controls its own rate; other connected clients are unaffected. Combined with latest-first delivery, a preview on a constrained link can request a low rate and still always show the current state of the page.
