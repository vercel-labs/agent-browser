...
The server sends cursor updates for mouse input dispatched by the agent or dashboard:
...
Coordinates use the session viewport coordinate space. `buttons` is the CDP button bitmask. `pressed` is true only for the event that starts a press, so a renderer can preserve a click animation even when press and release messages arrive between frames. The built-in dashboard uses these messages to draw its synthetic pointer and click ripple without modifying the page.
