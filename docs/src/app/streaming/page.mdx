...
The same port also serves the session preview UI and a small JSON API. Every `/api/` route is restricted to the session's own origin, so a page on another origin can neither drive nor read the session:
