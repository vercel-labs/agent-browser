...
The server samples the browser's actual computed cursor under the pointer and
broadcasts sanitized cursor updates:
...
`cursor` is always a standard CSS cursor keyword. Custom `url(...)` cursor
values are reduced to their fallback keyword, and unknown values become
`default`.
...
Use `buttons` when sending drag gestures. The stream server also tracks the
active button state from `mousePressed` and `mouseReleased`, so simple clients
can omit it for basic clicks.
...
For advanced stream clients, import the shared protocol types and helpers:

```typescript
import type { StreamMessage } from "agent-browser/client";
import { cdpButton, cdpButtonsForEvent, sanitizeCursorKeyword } from "agent-browser/client";

const ws = new WebSocket("ws://localhost:9223");
ws.onmessage = (event) => {
  const message = JSON.parse(event.data) as StreamMessage;
  if (message.type === "cursor") {
    document.body.style.cursor = sanitizeCursorKeyword(message.cursor);
  }
};
```

For lower-level browser control, use the TypeScript API:
