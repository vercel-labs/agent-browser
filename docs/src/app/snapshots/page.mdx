...
Runs of more than 50 consecutive siblings sharing the same role (like a huge `select` dropdown or a long list) collapse into a single summary line, e.g. `… 349 more "option" siblings omitted`. Refs of hidden siblings stay valid, so `select`, `find text`, and `--max-siblings 0` all keep working. Tune with `--max-siblings <n>` or `AGENT_BROWSER_SNAPSHOT_MAX_SIBLINGS`.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>--max-siblings</code></td><td>Collapse runs of consecutive same-role siblings beyond this count into a summary line (default 50, or <code>AGENT_BROWSER_SNAPSHOT_MAX_SIBLINGS</code>; 0 shows all)</td></tr>
  </tbody>
</table>
