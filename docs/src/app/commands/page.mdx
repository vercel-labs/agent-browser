...
Install or validate the current Chrome for Testing stable version. Add `--with-deps` to install Linux system dependencies. Add `--prune` to remove obsolete agent-browser-managed Chrome versions after successful validation.
...
Pruning is opt-in, refuses while sessions are active, and preserves the current manifest version plus unknown entries, symlinks, files, other engines, and user state. See [Pruning old Chrome versions](/installation#pruning-old-chrome-versions) for the full safety boundary.
