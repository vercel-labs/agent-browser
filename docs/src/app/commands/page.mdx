...
agent-browser runtime login https://example.com --attachable # Manual login with DevTools left on for later attach
...
<code>runtime login --attachable</code> followed by <code>runtime attach</code>.
