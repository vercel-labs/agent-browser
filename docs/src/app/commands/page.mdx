...
When `EXA_API_KEY` is set, the AI gains an `exa_web_search` tool. It uses [Exa](https://exa.ai) to search the web instead of navigating to a search engine, returning clean results with titles and text snippets.
