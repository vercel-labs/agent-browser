...
Open the dashboard through `http://localhost:4848` or a forwarded/proxied dashboard URL, such as the Portless URL `https://dashboard.agent-browser.localhost`. The browser stays on the dashboard origin; per-session tabs, status, and stream traffic are proxied internally, so session ports do not need to be exposed.
