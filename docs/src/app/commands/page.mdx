...
Normal browser commands, including `open`, `snapshot`, `click`, `fill`, `wait`, and tab switches, include the current WebMCP catalog with descriptions and input schemas. JSON exposes it as `data.webmcp`; CLI and MCP text content include it automatically. Invoke relevant tools directly, and use `webmcp list --json` when metadata is truncated. See [WebMCP](/webmcp) for context limits and refresh behavior.
