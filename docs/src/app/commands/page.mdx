...
Actions run through the normal command pipeline as `click @eN`, `fill @eN`, `scroll`, and `wait`, so action policies, `--confirm-actions`, `--allowed-domains`, and session isolation apply unchanged. The run stops on `DONE`, `BLOCKED`, the step budget, the time budget, or three consecutive actions that do not change the page. The exit code is `0` only on `DONE`. `DONE` is the model's opinion, not proof, so verify the outcome with `snapshot`, `get url`, or a screenshot. Start from a page that is already open; `goal` does not navigate on its own, and it cannot see inside iframes, shadow roots, or canvas.
...
`--debug` additionally writes every model request and reply to stderr, which is the quickest way to see what the model was offered on a step that went wrong. The `--json` result carries `status` (`done`, `blocked`, `timeout`, or `error`), the final `url`, `elapsedMs`, and one entry per step with the operation, the target ref, role and name, any typed text, the target probability, the model's confidence, the operation distribution, and per-phase timings.
