...
You can also assign a memorable label (`docs`, `app`, `admin`) at tab-creation time and use it anywhere an id is accepted. A label must start with a letter and contain only letters, digits, `-`, and `_`, and it can't take the `t<N>` id form (`t1`), which would make the labelled tab unreachable:
