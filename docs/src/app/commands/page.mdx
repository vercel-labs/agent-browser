...
Credential providers declaring `credential.challenge` may resolve a TOTP after an OTP field appears. Staged TOTP uses the same browser modes and selector behavior as the rest of `auth login`: a caller override takes precedence over an optional provider selector, followed by common OTP selectors including one-time-code metadata and a `123456` placeholder. An optional opaque challenge reference from the primary response is returned unchanged to the same provider. agent-browser fills the field without submitting it and returns `loggedIn: false` with `status: "challenge_filled"`.
...
- `--otp-selector <sel>`: staged OTP selector override for this login
