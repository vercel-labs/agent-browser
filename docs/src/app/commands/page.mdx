...
A bare positional that parses as a number is the milliseconds shorthand, so `wait 5000` sleeps. Pass `--selector` (or `-s`) whenever the selector is built from page data and could be all digits, such as a numeric id or an all-digit `data-testid`. The MCP tool `agent_browser_wait_for_selector` always uses `--selector`, so a selector there is always a selector.
