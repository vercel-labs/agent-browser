...
A screenshot's output path extension picks the encoder: `.png` writes PNG, `.jpg` and `.jpeg` write JPEG, and a `.webp` path is rejected because the encoder cannot produce WebP. `--screenshot-format` applies only when the path has no image extension, or when no path is given; a conflicting flag is ignored with a warning.
...
--screenshot-format <fmt> # Format: png (default), jpeg; only when the path has no image
                          # extension (or AGENT_BROWSER_SCREENSHOT_FORMAT)
