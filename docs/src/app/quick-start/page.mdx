...
The `select` command works with native `<select>` controls and standard ARIA `combobox` or `listbox` widgets. It matches exact values or labels, waits for associated custom options to render, and verifies the committed state. Use one value for single-select controls; multiple values require an explicitly multiselectable ARIA listbox. The action uses CDP-managed browser sessions; WebDriver backends report it as unsupported.
