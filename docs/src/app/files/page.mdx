...
agent-browser screenshot --screenshot-quality 80 ./page.jpg
agent-browser pdf ./page.pdf
```

The output path extension picks the encoder, so the bytes always match the file name: `.png` writes PNG, `.jpg` and `.jpeg` write JPEG, and a `.webp` path is rejected because the encoder cannot produce WebP. `--screenshot-format` applies only when the path has no image extension, or when no path is given; a conflicting flag is ignored with a warning.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>--screenshot-format</code> / <code>AGENT_BROWSER_SCREENSHOT_FORMAT</code></td><td><code>png</code> or <code>jpeg</code>, used only when the output path has no image extension</td></tr>
