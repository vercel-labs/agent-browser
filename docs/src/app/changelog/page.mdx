...
- **Steel cloud browser provider** - Connect to Steel sessions with `-p steel` / `AGENT_BROWSER_PROVIDER=steel`
- **Steel session options via env vars** - Configure timeout, CAPTCHA solving, proxy usage, region, profile persistence, and device mode
