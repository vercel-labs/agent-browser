...
Use `--isolate-context` when named sessions sharing one Chrome need separate cookies, storage, cache, and targets:
...
The context is sticky per session and survives daemon restarts. Explicit `close` disposes only that session's context. If Chrome restarts, agent-browser creates a clean replacement, never adopts a default-context tab, reapplies configured state, and reports <code>lifecycle.contextRecreated: true</code> in JSON. Omit the flag when you intentionally want to reuse the attached Chrome's login state.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>--isolate-context</code></td><td>Use a session-owned BrowserContext for cookie, storage, cache, and target isolation</td></tr>
