...
Sessions also keep their hands off the window itself: tabs are created in the background and switching to one skips `Page.bringToFront`, so driving a shared browser does not pull it over the app the person is using. `bringtofront` is there when a raise is what you actually want, and a screenshot of a background tab needs it.
