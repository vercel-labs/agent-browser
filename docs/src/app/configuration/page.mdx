...
`caCert` adds the supplied CA certificate or PEM bundle to CLI HTTPS/WSS trust. A direct `read URL` uses it without launching a browser or setting up NSS. `install` runs without a daemon and uses only its invocation's flags, environment, and config.

Daemon commands retain the session's CLI selection when flags, environment, and config supply no replacement. Each HTTP or WSS client acquisition rereads the selected bundle and, when enabled, native roots. Changed roots replace cached clients without restarting the daemon or browser; operations that already acquired a client may finish with its previous roots. Invalid explicit bundles fail instead of silently falling back. Set `clearCaCert: true` to clear the extra bundle and suppress its `SSL_CERT_FILE` fallback. Set `useSystemCa` to `false` to select built-in roots; this keeps any extra bundle unless it is also cleared.

For locally launched Chromium on Linux, `caCert` also imports each certificate into an isolated NSS database used only by that browser process. Different browser CA content or `clearCaCert: true` relaunches Chromium without restarting the daemon; equivalent content reuses Chromium. `agent-browser install --with-deps` installs the required `certutil`; otherwise install `libnss3-tools` on Debian/Ubuntu or `nss-tools` on RPM Linux. Browser-side trust does not support Chrome profiles, Lightpanda, macOS, or Windows. With CDP attach, auto-connect, or providers, `caCert` affects only CLI TLS and cannot change the remote browser's trust store. Set `useSystemCa` when the CA already lives in the operating system trust store.
...
`ignoreHttpsErrors` does not disable verification for CLI HTTPS/WSS, `read`, or `install`.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>AGENT_BROWSER_CLEAR_CA_CERT</code></td><td>Clear the session's extra CLI CA bundle; browser commands also clear local Chromium CA trust.</td><td>(disabled)</td></tr>
