...
A staged TOTP provider declares both `credential.read` and `credential.challenge`. See [Staged TOTP providers](/plugins#credential-plugin) for its optional selector, opaque challenge reference, and second request.
