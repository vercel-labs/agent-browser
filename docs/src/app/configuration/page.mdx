...
If you do not set <code>profile</code>, agent-browser uses the managed default profile at <code>~/.agent-browser/profile</code>. Set <code>profile</code> only when you want a different persistent directory or to reuse an existing Chrome profile by name.
