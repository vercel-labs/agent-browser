...
`caCert` trusts only that CA and keeps certificate and hostname verification on. Without the CA certificate on hand, fall back to ignoring every certificate error, which is less secure:
