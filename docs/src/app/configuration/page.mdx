...
A staged TOTP provider declares both `credential.read` and `credential.challenge`. See [Staged TOTP providers](/plugins#credential-plugin) for its optional selector metadata and second request.
