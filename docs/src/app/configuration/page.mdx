...
For wrapper launches (for example, WSL -> Windows Chromium), `launchCommand` can wrap the browser launch while `profile` still points at the browser user-data directory used for auto-connect discovery:
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>launchCommand</code></td><td><code>--launch-command</code></td><td>string</td></tr>
...
<table>
  ...
  <tbody>
    <tr><td><code>AGENT_BROWSER_AUTO_CONNECT</code></td><td>Auto-discover and connect to a running Chrome instance. If <code>AGENT_BROWSER_PROFILE</code> is set, checks <code>&lt;profile&gt;/DevToolsActivePort</code> first.</td><td>(disabled)</td></tr>
    ...
    <tr><td><code>AGENT_BROWSER_EXECUTABLE_PATH</code></td><td>Custom browser executable path.</td><td>(auto-detected)</td></tr>
    <tr><td><code>AGENT_BROWSER_LAUNCH_COMMAND</code></td><td>Shell command template used to launch the browser. Supports <code>&#123;executablePath&#125;</code> and <code>&#123;arguments&#125;</code>.</td><td>(none)</td></tr>
