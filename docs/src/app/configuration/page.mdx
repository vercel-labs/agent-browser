...
launch defaults, auth session name, and service login hints. A service with
...
<code>runtime login</code> or the attachable manual-login flow for services such
as Google. The nested <code>preferences</code> section is accepted as
forward-compatible config for future profile preferences.
