...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>allowedDomains</code></td><td><code>--allowed-domains</code></td><td>string[]; must list at least one domain (an empty array is rejected); also disables WebRTC in supported Chromium sessions and requires a fresh controllable browser context without profile/session startup args, restore/state replay, or direct-page provider plugins</td></tr>
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>AGENT_BROWSER_ALLOWED_DOMAINS</code></td><td>Comma-separated allowed domain patterns (e.g., <code>example.com,*.example.com</code>). An empty value is rejected instead of disabling containment; unset the variable to run unrestricted. Requires a fresh controllable browser context without profile/session startup args, restore/state replay, or direct-page provider plugins.</td><td>(unrestricted)</td></tr>
