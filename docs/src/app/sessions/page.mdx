...
Use `--tab-name <name>` when you want multiple independent tabs inside the same session and daemon. Named tabs share browser cookies and storage, but each name keeps its own element refs, active frame, and cross-origin iframe sessions.
