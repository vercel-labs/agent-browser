...
Browser commands within a session are serialized to preserve tab, frame, and element-reference consistency. A concurrent command receives a bounded JSON error with <code>code: "session_busy"</code>; retry it after the active command completes. `close` is a priority recovery operation that can cancel an unresponsive command, make a bounded best-effort restore save, and clean up daemon-owned resources. A user-attached CDP browser is disconnected but never terminated.
