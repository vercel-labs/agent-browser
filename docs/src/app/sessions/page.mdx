...
Named sessions normally reuse the attached Chrome's default login state. Add `--isolate-context` when sessions sharing one Chrome need separate cookies, storage, cache, and targets:
...
The context survives daemon restarts. Explicit `close` disposes only that session's context. A Chrome restart creates a clean replacement, reapplies configured state, and reports <code>lifecycle.contextRecreated: true</code>. Omit the flag when the session should reuse the attached Chrome's login state.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>AGENT_BROWSER_ISOLATE_CONTEXT</code></td><td>Use a session-owned BrowserContext for shared Chrome isolation</td></tr>
