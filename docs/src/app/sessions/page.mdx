...
If you do not pass <code>--profile</code>, agent-browser uses a stable Chrome user-data-dir at <code>~/.agent-browser/profile</code>. This keeps manual sign-in state across runs without any extra setup.
...
If you need a different persistent profile or multiple isolated Chrome instances at the same time, pass <code>--profile &lt;path&gt;</code> explicitly.
...
> <strong>Important:</strong> Do not treat <code>--profile &lt;name&gt;</code> as the preferred path for Google, Gmail, or other security-sensitive consumer properties. Because agent-browser launches from a copied temp snapshot of the named Chrome profile, those sites may detect an inconsistent browser state and reject sign-in. For those sites, prefer the default managed profile at <code>~/.agent-browser/profile</code> or <code>--auto-connect</code> to a real running Chrome.
