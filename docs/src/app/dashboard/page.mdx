...
The dashboard connects to the same WebSocket endpoint used by [Streaming](/streaming), including live frames, browser-derived cursor updates, and input injection. It also consumes additional message types for observability:
...
These are in addition to the existing `frame`, `status`, `cursor`, and `error` message types documented on the [Streaming](/streaming) page. The dashboard uses the shared `@agent-browser/client` protocol types so host apps can embed the same live-view stream without copying dashboard internals.
