...
These are in addition to the existing `frame`, `status`, `cursor`, and `error` message types documented on the [Streaming](/streaming) page. Host apps can consume the same shared protocol from `agent-browser/client` to embed the live-view stream without copying dashboard internals.
