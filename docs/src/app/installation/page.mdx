...
> **Note:** `npx` routes through Node.js before reaching the Rust CLI, so it is noticeably slower than a global install. For regular use, install globally. `bunx` is not supported because bun does not execute `postinstall` scripts.
