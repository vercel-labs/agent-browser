...
agent-browser-priv install  # Download Chrome from Chrome for Testing (first time)
```
...
```bash
npx agent-browser-priv install   # Download Chrome (first time only)
...
npx agent-browser-priv install  # Download Chrome (first time)
```
...
agent-browser-priv install  # Download Chrome (first time)
```
...
<tbody>
<tr><td>Chrome missing</td><td>Runs <code>agent-browser-priv install</code></td></tr>
