...
Community plugins are maintained outside the agent-browser repository. Review a plugin's source and security guidance before installing it.

- [CapSolver](https://github.com/tommycapsolver/agent-browser-plugin-capsolver) implements `captcha.solve`, reads its API key from the environment, and returns the provider's structured solution without taking control of browser navigation.
