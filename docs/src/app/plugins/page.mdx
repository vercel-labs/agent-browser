...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>credential.challenge</code></td><td><code>credential.resolve.challenge</code></td><td>Staged <code>auth login</code></td><td><code>challenge</code></td></tr>
...
For staged TOTP, declare both `credential.read` and `credential.challenge`. A credential that needs a second step includes `stagedChallenge`; its selector is optional:
...
```json
{
  ...
  "credential": {
    ...
    "stagedChallenge": {
      "kind": "totp",
      "selector": "#otp",
      "challengeRef": "provider-defined-opaque-value"
    }
  }
}
```

agent-browser waits for the OTP field before sending the same configured plugin a second request. Selector precedence matches username and password: `--otp-selector`, then the optional provider selector, then common OTP selectors including one-time-code metadata and a `123456` placeholder. The request origin is derived from the current page rather than provider metadata. If the primary response includes an opaque `challengeRef`, agent-browser returns it unchanged to the same provider:

```json
{
  "protocol": "agent-browser.plugin.v1",
  "type": "credential.resolve.challenge",
  "capability": "credential.challenge",
  "request": {
    "profileName": "my-app",
    "kind": "totp",
...
Return a non-empty ASCII-digit code and its expiry:
...
```json
{
  ...
  "challenge": {
    "kind": "totp",
    "code": "481516",
    "expiresAtUnixMs": 1785513630000
  }
}
```

Immediately before filling, agent-browser rejects a challenge whose `expiresAtUnixMs` has passed. It uses the same browser modes, caller overrides, and selector-based interaction path as username and password. It does not submit the OTP field or return the code.
