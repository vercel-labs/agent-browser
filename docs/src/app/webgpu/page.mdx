...
On Linux, `--webgpu` requires `--use-angle=vulkan`. A different ANGLE backend supplied through `--args` or `AGENT_BROWSER_ARGS` produces a launch error; the matching Vulkan value is allowed. Remove the custom switch to use the preset, or pass `--webgpu false` to choose another backend. Repeated `--disable-features` switches are merged with the built-in `Translate` opt-out.
