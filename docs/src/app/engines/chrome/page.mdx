...
If Chrome is not found, run `agent-browser install chrome` to download Chrome from Chrome for Testing.

## Usage

Chrome is the default engine, so no `--engine` flag is needed:
