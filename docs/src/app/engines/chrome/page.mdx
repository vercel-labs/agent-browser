...
If Chrome is not found, run `agent-browser install` to download Chrome from Chrome for Testing. To use a mirror, set both `AGENT_BROWSER_CHROME_LAST_KNOWN_GOOD_URL` and `AGENT_BROWSER_CHROME_DOWNLOAD_BASE_URL`.
