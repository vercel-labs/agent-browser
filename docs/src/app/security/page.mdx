...
The allowlist is sticky for the session. It is stored alongside the session's daemon files and re-applied whenever the daemon restarts, so an idle shutdown, an upgrade, a crash, or a later command that needs different daemon options cannot quietly resume the session without containment. A daemon that cannot read or store the session allowlist refuses to start and reports the file it could not use, instead of serving the session unrestricted. Only an explicit empty value drops the allowlist again.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>AGENT_BROWSER_ALLOWED_DOMAINS</code></td><td>Comma-separated allowed domain patterns; requires a fresh controllable browser context without profile/session startup args, restore/state replay, or direct-page provider plugins; sticky per session and re-applied after daemon restarts</td></tr>
