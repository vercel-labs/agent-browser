...
- **Unauthorized navigation / data exfiltration**: A compromised or manipulated agent could navigate to untrusted domains to exfiltrate data. Domain allowlists can restrict navigation, sub-resource requests, WebSocket connections, EventSource streams, and `sendBeacon` calls.
...
Restrict which domains the browser can navigate to and which domains pages can load resources from.
...
The legacy `allowedDomains` list restricts both navigation and sub-resource requests. For split control, use `navigationDomains` to restrict agent-initiated navigation and `resourceDomains` to restrict page-initiated resources such as scripts, images, fetch, XHR, WebSocket, EventSource, and beacons:

```bash
agent-browser --navigation-domains "app.example.com,*.app.example.com" open https://app.example.com
agent-browser --resource-domains "app.example.com,*.app.example.com,cdn.example.com" open https://app.example.com
```
...
Supports exact match (`github.com`) and wildcard prefix (`*.example.com`, which also matches the bare domain `example.com`). Non-http(s) sub-resources such as data URIs and blobs are still allowed. When a request is blocked, the command returns an error.

> **Note:** The WebSocket/EventSource blocking is best-effort because it works by overriding the browser constructors via an init script. If the `eval` action category is allowed, page scripts could theoretically restore the original constructors. For maximum protection, deny the `eval` category via `--action-policy` when using domain allowlists.
...
```json
{
  ...
  "resourceDomains": ["app.example.com", "*.app.example.com", "cdn.example.com"]
}
```

> **CDN and third-party resources:** When `allowedDomains` or `resourceDomains` restricts resources, sub-resource requests to non-allowed domains are blocked. Most websites load assets from CDN domains. Include these in your resource allowlist or pages will break. For example:
>
> ```bash
> --resource-domains "app.example.com,*.app.example.com,cdn.jsdelivr.net,fonts.googleapis.com,fonts.gstatic.com"
> ```

## Action Policy

Gate actions using a static policy file. The policy is enforced by the daemon, and denied actions fail immediately.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>AGENT_BROWSER_ALLOWED_DOMAINS</code></td><td>Comma-separated allowed domain patterns for navigation and resources</td></tr>
    <tr><td><code>AGENT_BROWSER_NAVIGATION_DOMAINS</code></td><td>Comma-separated navigation-only allowed domain patterns</td></tr>
    <tr><td><code>AGENT_BROWSER_RESOURCE_DOMAINS</code></td><td>Comma-separated resource-only allowed domain patterns</td></tr>
...
```json
{
  ...
  "maxOutput": 50000,
  "navigationDomains": ["your-app.com", "*.your-app.com"],
