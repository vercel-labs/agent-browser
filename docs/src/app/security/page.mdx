...
- **One pending confirmation per session.** A newer confirmation request replaces the previous one. Missing or stale IDs are rejected without consuming the current action; use the exact ID returned by the intended request.
...
The confirmation ID is required and must match the current pending command before it can be executed or discarded. Request IDs use UUIDs rather than reusable clock fragments; treat them as opaque and do not construct or truncate them. Missing or stale IDs fail without browser work or consuming a newer pending action. A matched `confirm` re-enters the action's normal policy and browser checks.

Explicit-URL HTTP reads negotiate support for this protocol on the same daemon connection before queuing the request. A same-version label is not sufficient; unsupported runtimes receive no read request and are not restarted or reconfigured. Only these HTTP confirmation prompts carry `data.capabilities.readRequiresConfirmation: true`. Bare DOM reads do not carry the marker, and unmarked legacy prompts must not be treated as proof of a read-only confirmation.

The capabilities-only `session_info` handshake exposes static protocol facts, not session data, and does not require permission for full session diagnostics. Full `session_info` and the subsequent read/action retain their normal policy checks.
