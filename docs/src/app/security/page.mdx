...
- **Domain filter timing on cloud providers.** Cloud providers run outside the local Chrome process, so resources loaded by provider-side startup behavior before agent-browser connects are not retroactively blocked. agent-browser rejects pre-existing browser attach modes, Chrome profiles, and raw startup Chrome args when the allowlist is active.
...
- **Pre-existing attach modes, profiles, and startup args do not support the domain allowlist.** agent-browser rejects `--allowed-domains` with `--cdp`, `--auto-connect`, `--profile`, and raw Chrome args that select profiles, restore sessions, or open startup pages because pages may have run script before agent-browser can replace WebRTC constructors.
- **iOS and Safari providers do not support the domain allowlist.** agent-browser rejects `--allowed-domains` with these providers because their automation backends cannot install the Chromium network controls or enforce equivalent WebRTC containment.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>AGENT_BROWSER_ALLOWED_DOMAINS</code></td><td>Comma-separated allowed domain patterns; requires a fresh controllable browser context without profile/session startup args</td></tr>
