...
- **Pre-existing attach modes do not support the domain allowlist.** agent-browser rejects `--allowed-domains` with `--cdp` and `--auto-connect` because pages may have run script before agent-browser can replace WebRTC constructors.
...
Supports exact match (`github.com`) and wildcard prefix (`*.example.com`, which also matches the bare domain `example.com`). Both page navigations and sub-resource requests (scripts, images, fetch, XHR, etc.) to non-allowed domains are blocked. WebSocket and EventSource connections are blocked via constructor-level patching, and `sendBeacon` returns `false` for blocked destinations. WebRTC peer connections are disabled entirely in supported Chromium sessions while the allowlist is active because STUN, TURN, and DNS traffic do not pass through CDP Fetch interception. Locally launched Chrome also uses `disable_non_proxied_udp` as a second layer. Pre-existing CDP sessions, auto-connect, iOS, and Safari reject this option because agent-browser cannot install equivalent containment before page scripts run. Non-http(s) sub-resources such as data URIs and blobs remain allowed. When a request is blocked, the command returns an error.
