...
A credential plugin may also declare `credential.challenge`. After an OTP selector resolves, agent-browser derives the current page origin, returns any opaque challenge reference unchanged to the same provider, requests a current code with `credential.resolve.challenge`, and fills the selector through the normal `auth login` interaction path without submitting or returning the code. The caller may override an optional provider selector, and common OTP selectors are used when neither supplies one. Concurrent external observers and capture surfaces retain their existing behavior.
...
agent-browser keeps browser automation and policy enforcement in core. Credential plugin values do not appear in command arguments or normal command output. Staged TOTP uses the existing capture behavior, so concurrent external observers may see the rendered field.
