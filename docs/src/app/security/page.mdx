...
An allowlist that is supplied but lists no domains is rejected. `--allowed-domains ""`, `--allowed-domains ","`, an empty `AGENT_BROWSER_ALLOWED_DOMAINS`, an empty `allowedDomains` config key, and an empty `allowedDomains` MCP array all fail with `Allowed domains list is empty.` instead of running without containment, which matters when the value comes from an environment variable or a config template that expands to nothing. Omit the option entirely when you intend to run unrestricted.
