...
- **Browser contexts** - First-class support for Chrome's `BrowserContext` primitive (CDP `Target.createBrowserContext`), enabling isolated cookies, localStorage, and cache between groups of tabs within a single Chrome process. New `agent-browser context new|list|close` commands with stable refs `c1`, `c2`, ... and optional `--label <name>`. New `tab new --context <ref|label> [url]` flag for creating tabs inside a specific context. Closing a context closes its tabs first; state save/load preserves per-context cookies and storage. Use cases: multi-identity workflows, A/B comparison (logged-in vs anonymous), sandboxed exploration, and parallel sub-agent isolation — all sharing one Chrome process (#1340)
